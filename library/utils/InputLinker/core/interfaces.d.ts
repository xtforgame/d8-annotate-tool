...
import { FieldName, FieldValue, FieldNormalizedValue, HostProps, RawEventArgs, StateMode, ValidReactElement, IlHost } from './types/commonTypes';
import { ProxyTypeName, ValueProxy, SliceName } from './types/proxyTypes';
import { IFieldLink, ValidateResult } from './types/fieldLinkInterfaces';
import { IlPresetMap, IlPreset, FieldObjectConfig, FieldConfig } from './types/fieldConfig';
export interface IFieldLinkClass {
    new <FieldLink extends IFieldLink<FieldLink>, InputLinker extends IInputLinker<FieldLink>>(linker: InputLinker, config: any): FieldLink;
}
export interface ValueProxyClass {
    new <FieldLink extends IFieldLink<FieldLink>, InputLinker extends IInputLinker<FieldLink>>(type: ProxyTypeName, linker: FieldLink): ValueProxy;
}
export interface ProxyAndSliceName {
    proxyName: ProxyTypeName;
    sliceName: SliceName;
}
export declare type PendingChange<FieldLink extends IFieldLink<FieldLink>> = {
    value: FieldValue;
    rawArgs: RawEventArgs;
    link: FieldLink;
};
export declare type PendingChangeCallback<FieldLink extends IFieldLink<FieldLink>> = Function;
export interface PendingChanges<FieldLink extends IFieldLink<FieldLink>> {
...
export declare type LinkerNamespace = string;
export interface IInputLinker<FieldLink extends IFieldLink<FieldLink>> {
    ...
    onFieldValueChange: (field: FieldLink, value: FieldValue, rawArgs: RawEventArgs, linkInfo: any) => void;
    ...
    evaluateConfig: (configWithQueue: ConfigWithLastQueue<FieldLink>, evaluatingConfig: FieldConfig<FieldLink>) => ConfigWithLastQueue<FieldLink>;
    ...
    getDataFromSliceByName: (sliceName: SliceName, fieldName: FieldName, state?: any) => FieldValue;
...
export interface IInputLinkerClass<FieldLink extends IFieldLink<FieldLink>, InputLinker extends IInputLinker<FieldLink>> {
    new <FieldLink extends IFieldLink<FieldLink>>(host: any, options: any): InputLinker;
}
